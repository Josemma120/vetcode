...
if ($action === 'request' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($res->num_rows > 0) {
        ...
        if ($stmt->execute()) {
            ...
            // Detectar automáticamente el dominio (localhost para pruebas, o tu web en hosting)
            $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
            $domainName = $_SERVER['HTTP_HOST'];
            $scriptPath = dirname($_SERVER['SCRIPT_NAME']); // Esto da la ruta actual (Controlador)
            // Ajustamos para salir de la carpeta Controlador y entrar a Vista
            $basePath = dirname($scriptPath); // Subimos un nivel
            $link = $protocol . $domainName . $basePath . "/Vista/restablecer.php?token=" . $token;
